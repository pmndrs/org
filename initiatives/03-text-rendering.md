...
Adequate solutions have existed for WebGL, but so far there are no WebGPU solutions that are built for mass distribution. On top of this, text rendering technology has evolved, including Slug going open source. All of this motivates us to create a text rendering solution that other libraries, such as UIKit, can build off of.
...
- Multiple rendering techniques, including Bitmap atlas, MSDF/MTSDF, and Slug-like approaches.
- Font and asset preprocessing and baking tools.
- Optional runtime font baking tools.
- Runtime glyph rendering and atlas management.
- OpenType text shaping and layout across modern scripts, with ligatures, kerning, bidi, line breaking, font fallback, vertical text, and scalable language-specific font bundles.
- Fast, dynamic Unicode-correct paragraph layout and measurement with shaping-safe wrapping, alignment, constraints, clipping, and ellipsis across styled text spans.
